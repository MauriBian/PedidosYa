...
class Bootstrap {
    ...
    val geo : Geo = Geo(12.0,12.0,"casa")
    ...
    val beto :Restaurant= Restaurant("Beto Burgers","hamburguesas","monteagudo 2300",geo,mp,menus)
    ...
    val menu5: Menu= Menu("Beto Hamburguesa Cheddar","simil cuarto de libra",productList5, OptionalDiscount.FIJO,10.0,true)
    ...
    val order1 : Order = Order(LocalDate.now(),ricardo,pato,menus2,State.DELIVERED,mp.first())
    ...
    val deliver = Deliver(pato.name,pato.code,mutableListOf(),PaymentMethods.Cash,geo)
    val deliver2 = Deliver("ricardo",8, mutableListOf(MenuID(5,1)),mp.first(),geo)
    init {

        application.customers.add(ricardo)
        application.customers.add(ortigotze)
        application.superUsers.add(superUser)
        application.superUsers.add(superUserPato)
        application.customers.addAll(customers)
        superUser.restaurant = beto
        superUserPato.restaurant = pato
        deliver2.code = order1.code
        ApplicationController.delivers.put(deliver,Rating(0))
        ApplicationController.delivers.put(deliver2,Rating(0))
